...
class TransferBase(object):
        ...
        for row in rows:
            try:
                allow = switchrule.isTurnOn(
                    row
                ) and row['enable'] == 1 and row['u'] + row['d'] < row['transfer_enable']
            except Exception as e:
                allow = False
            ...
            if allow:
                ...
                if not cfgchange and port in ServerPool.get_instance(
                    ...
                    for name in merge_config_keys:
                        if (name in cfg) and (
                            (name not in relay._config) or
                                not self.cmp(cfg[name], relay._config[name])):
                            cfgchange = True
                            break
            ...
            if port in mu_servers:
                ...
                elif not config['additional_ports_only'] and allow and port > 0 and port < 65536 and ServerPool.get_instance(
    ...
    def new_server(self, port, passwd, cfg):
        ...
        method = cfg.get(
            'method',
            ServerPool.get_instance().config.get('method', 'None'))
        obfs = cfg.get('obfs',
                       ServerPool.get_instance().config.get('obfs', 'plain'))
        logging.info(
    ...
    def del_servers():
        for port in [
            ...
            if ServerPool.get_instance().server_is_run(port) > 0:
                ServerPool.get_instance().cb_del_server(port)
        for port in [
                v for v in ServerPool.get_instance()
                .tcp_ipv6_servers_pool.keys()
        ]:
            if ServerPool.get_instance().server_is_run(port) > 0:
                ServerPool.get_instance().cb_del_server(port)
...
class Dbv3Transfer(DbTransfer):
    def __init__(self):
        ...
        self.update_node_state = True if get_config(
        ).API_INTERFACE != 'legendsockssr' else False
...
class MuJsonTransfer(TransferBase):
    ...
    def update_all_user(self, dt_transfer):
        ...
        if rows:
            output = json.dumps(
                rows, sort_keys=True, indent=4, separators=(',', ': '))
